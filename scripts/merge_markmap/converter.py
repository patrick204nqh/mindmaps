...
indent_size = formatting_settings.get(
    "indent_size", 2
)  # Default to 2 spaces if not set
line_spacing = formatting_settings.get(
    "line_spacing", 1
)  # Default to 1 line if not set
...
def convert_ast_to_markdown(
    ...
    if include_front_matter and front_matter:
        ...
        markdown_lines.extend(
            [""] * line_spacing
        )  # Apply line spacing after front matter

    if main_heading:
        markdown_lines.append(f"# {main_heading}")  # H1 heading
        markdown_lines.extend(
            [""] * line_spacing
        )  # Apply line spacing after the heading
        ...
        if merged_ast:
            ...
            markdown_lines.extend(
                [""] * line_spacing
            )  # Apply line spacing after the list
