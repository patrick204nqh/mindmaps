...
def extract_text(node):
    ...
    if isinstance(node, dict):
        ...
        if node_type == 'text':
            ...
            inner_text = ''.join(extract_text(child) for child in node.get('children', []))
            if node_type == 'strong':
                return f"**{inner_text}**"
...
def convert_ast_to_markdown(main_heading, merged_ast, front_matter=None, include_front_matter=False):
    ...
    if main_heading:
        ...
        if main_heading in merged_ast:
            markdown_lines.extend(convert_structured_lists_to_markdown(merged_ast[main_heading]))
    ...
    for heading, items in merged_ast.items():
        if heading != main_heading:
            ...
            markdown_lines.extend(convert_structured_lists_to_markdown(items, indent_level=1))
            markdown_lines.append("")  # Empty line for better formatting
